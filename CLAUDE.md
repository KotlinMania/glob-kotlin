# Claude Code Project Instructions — glob-kotlin

## Project Overview

This is **glob-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`glob`](https://crates.io/crates/glob).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/glob/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/glob>
...
Reference example: `tmp/serde/serde_core/src/private/mod.rs`
re-exports `Result` from `crate::lib::result`. The Kotlin tracking file lives at
`src/commonMain/kotlin/io/github/kotlinmania/serde/core/private/Mod.kt`.

...
fun installProjectAndroidSdk(execOperations: ExecOperations) {
    ...
    if (androidSdkInstallMarker.exists()) {
        println("setup-android-sdk: cached SDK is missing required packages; repairing $projectAndroidSdkDir")
    }
...
kotlin {
    ...
    val xcf = XCFramework("Glob")

    macosArm64 {
        binaries.framework { baseName = "Glob"; xcf.add(this) }
    }
    iosArm64 {
        binaries.framework { baseName = "Glob"; xcf.add(this) }
    }
    ...
    swiftExport {
        moduleName = "Glob"
        flattenPackage = "io.github.kotlinmania.glob"
    }

    android {
        namespace = "io.github.kotlinmania.glob"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "glob-kotlin", version.toString())

    pom {
        name.set("glob-kotlin")
        description.set("Kotlin Multiplatform port of rust-lang/glob - Support for matching file paths against Unix shell style patterns")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/glob-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/glob-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/glob-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/glob-kotlin.git")
        }
    }
}
    ...
    doFirst {
        ...
        if (sourceFiles.isEmpty()) {
            ...
            sentinelFile.writeText(
                ...
                package io.github.kotlinmania.glob.codeql
